...
datadir = 'raw_data/WolockMcCabe-WaterBalance/' + var +'/'  # directory containing csv files from https://www.sciencebase.gov/catalog/item/59c2b980e4b091459a61d425
...
outdir = 'wrk_data/WaterBalance/' + var + '/'  # directory for output rasters
...
for decade in range(roundDown(start_year, 10), roundDown(end_year, 10)+1, 10):  # data organized by decade
    fn = var + str(decade) + 's.csv'  # file name for decade of data
    ...
    for year in range(decade, decade+10):  # iterate through each year in a decade
        if (year >= start_year) and (year <= end_year) and (year in years):
            for month in range(1, 13):  # iterate through each year in a month
                cn = var + '_' + str(year) + str(month).zfill(2)  # column name containing runoff values for month of year
                ofn = var + '_' + str(year) + str(month).zfill(2) + '.tif'  # output raster (.tif) name
