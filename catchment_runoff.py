...
for year in years:
    ...
    for month in range(1, 13):
        fn = 'usa_grids/runoff_' + str(year) + str(month).zfill(2) + '.tif'  # file name to clip and save (in new dir)
        clipraster = basedir + fn  # path of raster to clip
        outraster = basedir + 'nhd17a_catchments_30m/temp.tif'  # path of clipped output
        ...
        # convert from mm*100 to m
        df_int = df_temp[[idfield, 'mean'+str(year)+str(month).zfill(2), 'sum'+str(year)+str(month).zfill(2)]]
        df_int[['mean'+str(year)+str(month).zfill(2), 'sum'+str(year)+str(month).zfill(2)]] = \
            df_int[['mean'+str(year)+str(month).zfill(2), 'sum'+str(year)+str(month).zfill(2)]]/100000.0
        if month == 1:
            df = df.append(df_int, ignore_index=True)
        else:
            df = df.join(df_int.set_index(idfield), on=idfield)

    df.to_csv(basedir + csvdir + 'catrun_' + str(year) + '_m.csv', index=False)
    yearct += 1
